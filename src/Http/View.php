...
class View extends BaseResponse
{
    ...
    public function with_message(string $message, string $type = Flash::FLASH_SUCCESS, array $options = []): View
    {
        if (!$message) {
            throw new \Exception('Message must be set.');
        }

        $this->flash_message = ['message' => $message, 'type' => $type, 'options' => $options];
        return $this;
    }
    ...
    public function render(): string
    {
        ...
        if (!empty($this->flash_message)) {
            flash('flash_' . uniqid(), $this->flash_message['message'], $this->flash_message['type'], $this->flash_message['options']);
        }
